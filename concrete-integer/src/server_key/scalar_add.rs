...
impl ServerKey {
    ...
    /// ```
    pub fn unchecked_scalar_add(&self, ct: &Ciphertext, scalar: u64) -> Ciphertext {
    ...
    pub fn unchecked_scalar_add_assign(&self, ct: &mut Ciphertext, scalar: u64) {
    ...
    /// assert_eq!(true, res);
    /// ```
    pub fn is_scalar_add_possible(&self, ct: &Ciphertext, scalar: u64) -> bool {
        //Bits of message put to 1
        let mask = (self.key.message_modulus.0 - 1) as u64;
    ...
    /// assert_eq!(msg + scalar, dec);
    /// ```
    pub fn smart_scalar_add(&self, ct: &mut Ciphertext, scalar: u64) -> Ciphertext {
        if !self.is_scalar_add_possible(ct, scalar) {
