...
impl ServerKey {
    ...
    /// ```
    pub fn unchecked_crt_scalar_sub(&self, ct: &Ciphertext, scalar: u64) -> Ciphertext {
        let mut result = ct.clone();
        self.unchecked_crt_scalar_sub_assign(&mut result, scalar);
        result
    }

    pub fn unchecked_crt_scalar_sub_assign(&self, ct: &mut Ciphertext, scalar: u64) {
    ...
    /// assert_eq!(true, bit);
    /// ```
    pub fn is_crt_scalar_sub_possible(&self, ct: &Ciphertext, scalar: u64) -> bool {

        for (ct_i, mod_i) in ct.ct_vec.iter().zip(ct.message_modulus_vec.iter()) {
    ...
    /// assert_eq!((clear_1 - clear_2) % 30, res);
    /// ```
    pub fn smart_crt_scalar_sub(&self, ct: &mut Ciphertext, scalar: u64) -> Ciphertext {
        if !self.is_crt_scalar_sub_possible(ct, scalar) {
    ...
    pub fn smart_crt_scalar_sub_assign(&self, ct: &mut Ciphertext, scalar: u64) {
        if !self.is_crt_scalar_sub_possible(ct, scalar) {
            self.full_extract(ct);
        }
