...
impl ServerKey {
    ...
    /// ```
    pub fn unchecked_crt_scalar_add(&self, ct: &Ciphertext, scalar: u64) -> Ciphertext {
    ...
    pub fn unchecked_crt_scalar_add_assign(&self, ct: &mut Ciphertext, scalar: u64) {
    ...
    /// assert_eq!(true, tmp);
    /// ```
    pub fn is_crt_scalar_add_possible(&self, ct: &Ciphertext, scalar: u64) -> bool {
    ...
    /// assert_eq!((clear_1 + clear_2) % 30, res);
    /// ```
    pub fn smart_crt_scalar_add(&self, ct: &mut Ciphertext, scalar: u64) -> Ciphertext {
        if !self.is_crt_scalar_add_possible(ct, scalar) {
    ...
    /// assert_eq!((clear_1 + clear_2) % 30, res);
    /// ```
    pub fn smart_crt_scalar_add_assign(&self, ct: &mut Ciphertext, scalar: u64) {
        if !self.is_crt_scalar_add_possible(ct, scalar) {
            self.full_extract(ct);
        }
        self.unchecked_crt_scalar_add_assign(ct, scalar);
