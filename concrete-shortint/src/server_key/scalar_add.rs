...
impl ServerKey {
    ...
    pub fn unchecked_scalar_add_assign_crt(&self, server_key: &ServerKey, ct: &mut Ciphertext, scalar: u8) {
        ShortintEngine::with_thread_local_mut(|engine| {
            engine.unchecked_scalar_add_assign_crt(server_key, ct, scalar).unwrap()
        })
    }
